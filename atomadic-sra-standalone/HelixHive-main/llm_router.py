...
class KeyRotator:
    """
    Sovereign Key Rotator v4.4.1.0
    Rotates between primary and secondary keys on rate-limit failure.
    """
    def __init__(self, name: str, primary: str, secondary: Optional[str] = None):
        self.name = name
        self.keys = [primary] if primary else []
        if secondary:
            self.keys.append(secondary)
        
        self.current_idx = 0
        self.last_limit_hit = [0.0] * len(self.keys)
    ...
    def get_active_key(self) -> Optional[str]:
        ...
        for _ in range(len(self.keys)):
            if now - self.last_limit_hit[self.current_idx] > self.cooldown_period:
                return self.keys[self.current_idx]
            self.current_idx = (self.current_idx + 1) % len(self.keys)
            
        # All keys in cooldown, return current anyway as fallback
        return self.keys[self.current_idx]

    def mark_rate_limited(self):
        logger.warning(f"[KeyRotator] Key {self.current_idx} for {self.name} hit rate limit. Rotating...")
        self.last_limit_hit[self.current_idx] = time.time()
        if len(self.keys) > 1:
            self.current_idx = (self.current_idx + 1) % len(self.keys)
...
class OpenAICompatibleProvider(LLMProvider):
    ...
    async def complete(self, request: LLMRequest, model: Optional[str] = None) -> LLMResponse:
        ...
        api_key = self.rotator.get_active_key() if self.rotator else None
        if not api_key:
            raise Exception(f"No active API key for {self.name}")
        ...
        try:
            async with session.post(
                ...
                if resp.status == 429:
                    ...
                    raise Exception(f"Rate limited (429) on {self.name} – rotating key. Retry tokens in {retry_after}s")
...
class HelixLLMRouter:
    ...
    def _init_providers(self):
        ...
        # Load API keys with Rotators
        self.providers["groq"].rotator = KeyRotator(
            "groq", 
            os.getenv("GROQ_API_KEY"), 
            os.getenv("GROQ_API_KEY_SECONDARY")
        )
        self.providers["openrouter"].rotator = KeyRotator(
            "openrouter", 
